var api = "http://localhost:1337/api/graphicdesigns?populate[projects][populate]=thumbnail";
...
fetch(api)
  ...
  .then(data => {
    ...
    const graphicsDesignContainer = document.getElementById('graphicsdesign-container');

    if (data && data.data) {
      // Gehe durch alle Graphicdesign-Einträge und deren zugehörige Projekte
        ...
        if (entry.projects && entry.projects.length) {
          entry.projects
            ...
            .forEach(project => {
              ...
              graphicsDesignContainer.appendChild(projectElement);
            });
        }
      });
    } else {
      console.error('No graphics design data found.');
    }
  })
  .catch(error => {
